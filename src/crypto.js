...
import { createHmac, createVerify, createSign, timingSafeEqual, constants, sign, verify } from 'crypto'

import { joseToDer, derToJose } from 'ecdsa-sig-formatter'
import { TokenError } from './error'
import Cache from 'mnemonist/lru-cache'

const define = require('asn1.js').define
...
let directSign = sign
const directVerify = verify

export const useNewCrypto = typeof directSign === 'function'
...
const PrivateKey = define('PrivateKey', function () {
  this.seq().obj(
...
const PublicKey = define('PublicKey', function () {
  this.seq().obj(
    this.key('algorithm').seq().obj(this.key('algorithm').objid(), this.key('parameters').optional().objid())
  )
})
...
export function detectPrivateKeyAlgorithm(key) {
